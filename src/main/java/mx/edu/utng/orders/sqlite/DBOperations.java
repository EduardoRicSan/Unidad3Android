...
/**
 * Created by Jesús Eduardo Rico Sandoval on 7/02/17.
 */
...
public final class DBOperations {
    ...
    public Cursor getComments(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.COMMENT);
        return  database.rawQuery(sql, null);
    }

    public Cursor getCommentById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.COMMENT,
                OrderContract.Comments.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }
    ...
    public String insertComment(Comment comment){
        SQLiteDatabase database = db.getWritableDatabase();
        String idComment = OrderContract.Comments.generateIdComment();
        ContentValues values = new ContentValues();
        values.put(OrderContract.Comments.ID, idComment);
        values.put(OrderContract.Comments.COMMENT, comment.getComment());
        values.put(OrderContract.Comments.MSG, comment.getMsg());
        values.put(OrderContract.Comments.CREATED, comment.getCreated());
        database.insertOrThrow(OrderDB.Tables.COMMENT, null, values);
        return idComment;
    }
    ...
    public boolean updateComment(Comment comment){
        ...
        values.put(OrderContract.Comments.MSG, comment.getMsg());
        values.put(OrderContract.Comments.CREATED, comment.getCreated());
        String whereClause = String.format("%s=?", OrderContract.Comments.ID);
        String[] whereArgs = {comment.getId()};
        int result = database.update(OrderDB.Tables.COMMENT,
                values, whereClause, whereArgs);
        return result>0;
    }
    ...
    public boolean deleteComment(String id){
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = OrderContract.Comments.ID + "=?";
        String[] whereArgs = {id};
        int result =  database.delete(OrderDB.Tables.COMMENT,
                whereClause, whereArgs);
        return result > 0;
    }
